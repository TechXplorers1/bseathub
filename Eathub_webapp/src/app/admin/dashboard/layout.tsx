...
export default function AdminDashboardLayout({
        ...
        <nav className="grid items-start gap-1 px-2 text-sm font-medium lg:px-4">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`h-4 w-4 flex-shrink-0 ${active ? 'text-orange-600' : 'text-muted-foreground'
                    }`}
                />
  ...
  return (
    <div className="flex min-h-screen w-full flex-col md:flex-row overflow-hidden">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-background border-r transform transition-transform duration-250 ease-in-out md:static md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
          } flex flex-col`}
      >
