        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/20 pb-12">
                                                                        ...
                                                                        <DropdownMenu>
                                                                            ...
                                                                            <DropdownMenuContent align="end" className="rounded-xl shadow-xl w-48 p-2">
                                                                                ...
                                                                                {order.currentStatusId === 'Delivered' && (
                                                                                    <DropdownMenuItem 
                                                                                        className="font-bold rounded-lg text-amber-600"
                                                                                        onClick={() => setReviewDialog({ isOpen: true, order })}
                                                                                    >
                                                                                        Rate & Review
                                                                                    </DropdownMenuItem>
                                                                                )}
            ...
            {selectedOrder && (
                <OrderDetailsDialog
                    ...
                    onOpenReview={(order: OrderResponse) => setReviewDialog({ isOpen: true, order })}
                />
            )}

            {reviewDialog.isOpen && reviewDialog.order && (
                <ReviewDialog
                    isOpen={reviewDialog.isOpen}
                    onClose={() => setReviewDialog(prev => ({ ...prev, isOpen: false }))}
                    targetId={reviewDialog.order.sourceId}
                    targetType={reviewDialog.order.sourceType as any}
                    providerName={reviewDialog.order.sourceName}
                    orderedItems={reviewDialog.order.items.map(i => ({ itemRefId: i.itemRefId, itemName: i.itemName }))}
                    orderTotal={reviewDialog.order.totalAmount}
                    orderDate={new Date(reviewDialog.order.orderPlacedAt).toLocaleDateString()}
                    orderId={reviewDialog.order.id}
                />
            )}
