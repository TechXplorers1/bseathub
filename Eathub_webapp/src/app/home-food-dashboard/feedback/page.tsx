...
import { Loader2, MessageSquare, Star, Reply, Home } from 'lucide-react';
import {
  Card,
  CardContent,
  CardDescription,
...
export default function HomeFoodFeedbackPage() {
  ...
  useEffect(() => {
    const providerId = localStorage.getItem('homeFoodId');
    if (!providerId) {
      setLoading(false);
      return;
    }

    getReviewsForProvider(providerId, 'HomeFood')
  ...
  const handleReply = async (reviewId: string) => {
    ...
    try {
        ...
        toast({
            title: 'Message Sent',
            description: 'Your neighbor will see your heartwarming response!'
        });
    } catch (err) {
        toast({
            variant: 'destructive',
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-muted-foreground font-medium">Heartwarming notes from your local community.</p>
      </div>
      ...
      <Card className="border-none shadow-2xl bg-card/60 backdrop-blur-md overflow-hidden">
        <CardHeader className="bg-muted/30 border-b border-border/50 px-8 py-6">
          <CardTitle className="text-2xl font-bold">Latest Reviews</CardTitle>
          <CardDescription className="font-medium text-muted-foreground/80">
            {loading ? "Checking the comment box..." : `You have ${reviews.length} heartwarming reviews.`}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
             <div className="flex flex-col items-center justify-center py-24 gap-4">
                ...
                <p className="text-muted-foreground font-bold animate-pulse text-sm">Gathering community feedback...</p>
             </div>
          ) : reviews.length === 0 ? (
            <div className="py-24 text-center flex flex-col items-center gap-4">
                 ...
                 <p className="text-muted-foreground font-black text-xl">No reviews yet</p>
                 <p className="text-muted-foreground/70 text-sm font-medium max-w-xs mx-auto">When your neighbors love your home cooking, their feedback will shine here!</p>
            </div>
          ) : (
                ...
                <div key={review.id} className="p-8 hover:bg-muted/30 transition-colors group">
                  <div className="flex gap-6 items-start">
                    ...
                    <div className="flex-1">
                      ...
                      {review.reply ? (
                        <div className="mt-6 ml-4 p-4 rounded-2xl bg-primary/5 border border-primary/10 relative animate-in zoom-in-95 duration-500">
                        ...
                        <div className="mt-4 flex flex-col gap-2 animate-in slide-in-from-top-2 duration-300">
                             <Input 
                                placeholder="Add a note to your community..." 
                                value={replyText}
