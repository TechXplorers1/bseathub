...
import { login as apiLogin, sendOtp, verifyOtp, AUTH_URL, forgotPassword, resetPassword, googleLogin as apiGoogleLogin } from '@/services/api';
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!user.email) throw new Error("Google login failed: No email provided");
      ...
      if (response.avatarUrl) localStorage.setItem('userAvatar', response.avatarUrl);

      window.dispatchEvent(new Event('auth-change'));

      toast({
        title: "Google Login Successful",
        description: `Welcome, ${response.name || response.email}!`,
      });
