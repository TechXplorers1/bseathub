...
export default function PartnerPage() {
    ...
    const handleRegistrationSubmit = async (data: any, type: string) => {
        try {
            ...
            if (result) {
                toast({
                    ...
                    description: `Welcome! You are now registered as a ${result.role}.`,
                });

                // Close specific dialogs
                setIsHomeFoodDialogOpen(false);
        ...
        <div className="min-h-screen bg-orange-50/50 flex flex-col items-center justify-center p-8">
            <div className="text-center mb-12">
                ...
                <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
                    Join Eat Hub as a Restaurant, Home Food Cook, or Private Chef.
                </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
                {partnerOptions.map((option) => (
                    <Card key={option.id} className="text-center hover:shadow-xl transition-shadow duration-300 flex flex-col">
                        ...
                        <div className="p-6">
                            <Button onClick={() => handleRegisterClick(option.id)} className="w-full" size="lg">
                                Register Now
                            </Button>
                        </div>
                    </Card>
